...
app = Client(
    ...
    bot_token="6357128136:AAEjHDWbZDsFmHXYIHH8FBwn596dfc6zS0g",#توكنك
)
loop = get_event_loop()
...
async def start(_: Client, message: Message):
    ...
    if user_id == owner and users.get(str(user_id)) is None:
        ...
        write(users_db, users)
        return await message.reply(f"مرحبًا بك عزيزي في بوت النشر التلقائي ، لـ استخدام البوت عليك الإشتراك من المطور @vv9uu [الأســـــــعـــــــار](https://t.me/cvvvgy/2)")
    elif not users[str(user_id)]["vip"]: return await message.reply(f"مرحبًا بك عزيزي في بوت النشر التلقائي ، لـ استخدام البوت عليك الإشتراك من المطور @vv9uu [الأســـــــعـــــــار](https://t.me/cvvvgy/2)")
